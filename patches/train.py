"""
PATCH for src/train.py

WHAT CHANGED (search for '# PATCH' comments below):
  - run_lodo_fold() now unpacks the 3-tuple (X, y, subject_ids) that the
    patched scripts/04_run_lodo_experiment.py loader produces, instead of
    the old 2-tuple (X, y). Train-set subject_ids are concatenated too
    (not currently used downstream, but kept for future subject-grouped
    inner validation — see the Section 15 caveat in your own
    EXPERIMENT_LOG.md about the inner split not being subject-grouped).
  - run_lodo_fold() now returns y_true, y_prob, and test_subject_ids
    alongside the existing metrics/history — nothing existing is removed.

NO CHANGE to: model architecture, training loop, loss, optimizer,
early-stopping logic, class weighting, or any hyperparameter. This patch
touches only what data run_lodo_fold reads and what it returns.

HOW TO APPLY: replace src/train.py with this file's content in full.
"""
...
def run_lodo_fold(held_out_dataset, all_data, cfg, device):
    ...
    # PATCH: all_data[k] is now (X, y, subject_ids) — subject_ids may be
    # None for a given dataset if its cache predates the subject_id fix.
    ...
    X_test, y_test, test_subject_ids = all_data[held_out_dataset]
    ...
    # PATCH: carry y_true/y_prob/test_subject_ids through to the caller so
    # they can be persisted and reused for a no-retrain permutation test.
    return {
