            ...
            for fold in SW[str(seed)][: (2 if a.smoke else None)]:
                ...
                for model_name in a.models:
                    ...
                    if model_name in ("logreg", "svm_rbf"): p, info = E1.run_classical(model_name, d, tr, va, te, d["y"], seed); tids = d["tid"][te]
                        ...
                        p = m.predict_proba(sc.transform(d["X"][te]))[:, 1]; tids = d["tid"][te]; info = {**info, "p_train": m.predict_proba(sc.transform(d["X"][tr]))[:, 1]}; info = {**info, "p_train": m.predict_proba(sc.transform(d["X"][tr]))[:, 1]}
                    elif model_name == "mlp":
